            ...
            for count in range(len(payment_mode)):
                ...
                if paid_amount[count]:
                    ...
                    for i in range(4):
                        ...
                        if split_amount <= payment_amount:  # Use <= instead of < to ensure correct handling
                            payments = PaymentDetails()
                            payments.booking = book
                        ...
                        else:  # No need to check payment_amount > 0 separately if split_amount is handled
                            payments = PaymentDetails()
                            payments.booking = book
                            payments.payment_mode = payment_mode[count]
                            payments.bank = bank[count]
        ...
        if action == 'search_customer':
            ...
            try:
                ...
                if get_number:
                    ...
                    get_number = "63421"
                divide_value =int(book.total_site_value) / 4
                split_amount = int(divide_value)
                print('sp:',int(split_amount))
                paid_amount =  request.POST.getlist('amount[]')
                payment_mode = request.POST.getlist('payment_mode[]')
                bank = request.POST.getlist('bank[]')
                branch = request.POST.getlist('branch[]')
